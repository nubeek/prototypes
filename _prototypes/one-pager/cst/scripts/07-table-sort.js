...
function getPresentationRowStyle(rowIndex) {
  if (!isOnePagerPresentation) return "";
  return ` style="--presentation-row-index: ${Math.min(rowIndex, 24)};"`;
}
...
function renderLocations(rows) {
  ...
  tableBody.innerHTML = pageRows
    .map((row, pageRowIndex) => {
      ...
      return `
        <tr class="${row.isProspectDataset ? "prospect-dataset-row" : ""} ${isSelected ? "is-checked" : ""}"${getPresentationRowStyle(pageRowIndex)} ${getLocationRowAttributeMarkup(row, pageRowIndex)}>
