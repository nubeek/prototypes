...
function syncRadiusFilterControls() {
  ...
  if (radiusValueLabel) {
    radiusValueLabel.textContent = `${Math.round(selectedRadiusMiles)} mi`;
  }
...
function setRadiusValue(value, { refresh = false, preservePrecision = false } = {}) {
  const numericValue = Number(value);
  const normalizedValue = Math.min(
    RADIUS_FILTER_DEFAULTS.max,
    Math.max(
      RADIUS_FILTER_DEFAULTS.min,
      Number.isFinite(numericValue) ? numericValue : RADIUS_FILTER_DEFAULTS.value
    )
  );
  const nextValue = preservePrecision ? normalizedValue : Math.round(normalizedValue);
...
function enhanceFilterCombobox(select, { allowExclude = false } = {}) {
  ...
  let renderedOptions = [];
  let previousSelectedValueSet = new Set();
  let controlResizeFrame = null;
  let controlResizeCleanupTimeout = null;
  ...
  function animateControlResize(fromHeight, toHeight) {
    if (Math.abs(toHeight - fromHeight) < 0.5) return;

    if (controlResizeFrame !== null) {
      window.cancelAnimationFrame(controlResizeFrame);
      controlResizeFrame = null;
    }
    window.clearTimeout(controlResizeCleanupTimeout);

    const finishResize = () => {
      control.classList.remove("is-resizing");
    ...
    const handleTransitionEnd = (event) => {
      if (event.target !== control || event.propertyName !== "height") return;
      finishResize();
    };

    control.classList.add("is-resizing");
    control.style.height = `${fromHeight}px`;
    controlResizeFrame = window.requestAnimationFrame(() => {
      controlResizeFrame = window.requestAnimationFrame(() => {
        control.style.height = `${toHeight}px`;
      });
    });
    control.addEventListener("transitionend", handleTransitionEnd);
    controlResizeCleanupTimeout = window.setTimeout(finishResize, 360);
  }

  function syncComboboxDisplay() {
    const previousControlHeight = control.getBoundingClientRect().height;
    ...
    selectedOptions.forEach((option) => {
      ...
      const isNewSelection = !previousSelectedValueSet.has(option.value);
    ...
    previousSelectedValueSet = selectedValueSet;
    ...
    const nextControlHeight = control.getBoundingClientRect().height;
    animateControlResize(previousControlHeight, nextControlHeight);
  }
