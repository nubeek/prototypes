...
function syncCreateTargetVisibilityFieldState() {
  if (!createTargetVisibilitySelect) return;
  const selectField = createTargetVisibilitySelect.closest(".target-modal-select-field");
  if (!selectField) return;
  selectField.classList.toggle("is-private", createTargetVisibilitySelect.value === "private");
}
...
function openCreateTargetModal(trigger = null) {
  ...
  createTargetForm?.reset();
  syncCreateTargetVisibilityFieldState();
...
if (createTargetVisibilitySelect) {
  createTargetVisibilitySelect.addEventListener("change", syncCreateTargetVisibilityFieldState);
  syncCreateTargetVisibilityFieldState();
}
