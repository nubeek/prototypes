...
const TERRITORY_INFO_CARD_FALLBACK_HEIGHT = 340;
...
function bindTerritoryInfoCard() {
  ...
  if (primaryClose && primaryClose.dataset.bound !== "true") {
    ...
    primaryClose.addEventListener("click", () => {
      clearSelectedTerritory({ closeMapPanel: true });
    });
  }
...
function updateConsolidatedSharedTerritories(territoryMap, occupantsByGeoKey) {
  ...
  setTerritoryLayerFilter(territoryMap, TERRITORY_SHARED_ALL_HIT_LAYER_ID, hiddenSharedFilter);
...
async function loadTerritoryData(territoryMap) {
  try {
    ...
    addTerritoryDensityLayers(
      territoryMap,
      brands.length ? `territories-${brands[0].id}-fill` : null
    );
...
function selectTerritory(brandId, stateCode) {
