...
function getCurrentTerritorySettings() {
  ...
  return {
    ...
    settings: {
      ...
      brandCircles: document.getElementById("territoryBrandCirclesToggleOption")?.getAttribute("aria-checked") === "true",
...
function restoreSavedFilterSelections(settings) {
  ...
  setFilterSelectIncludedExcludedValues(
    ...
    getValidSavedSelectValues(locationFilterSelect, savedLocations.included),
    getValidSavedSelectValues(locationFilterSelect, savedLocations.excluded)
  );
  setFilterSelectIncludedExcludedValues(
...
function applySavedMapSettings(settings = savedTerritorySettings) {
  ...
  window.territoryMapControls?.setTerritoryBrandCirclesVisible?.(mapSettings.brandCircles !== false);
  window.territoryMapControls?.setTerritoryBordersVisible?.(mapSettings.borders !== false);
  const densityEnabled = Boolean(mapSettings.density);
  window.territoryMapControls?.setTerritoryBlendEnabled?.(
    Boolean(mapSettings.blend) && !densityEnabled,
...
function clearFilterSection(section) {
  ...
  const locationFilterSelect = section.querySelector("#locationFilterSelect");
...
function initTerritoryFilters() {
  ...
  document.querySelectorAll(".filter-field-select").forEach((select) => {
    const allowExclude = select.id === "locationFilterSelect"
...
function initTerritoryToolbarMenu() {
  ...
  const territoryBrandCirclesToggle = document.getElementById("territoryBrandCirclesToggleOption");
  ...
  let territoryBrandCirclesEnabled = savedSettings?.brandCircles ?? window.territoryMapControls?.getTerritoryBrandCirclesVisible?.() ?? true;
  let territoryBordersEnabled = savedSettings?.borders ?? window.territoryMapControls?.getTerritoryBordersVisible?.() ?? true;
  let territoryDensityEnabled = savedSettings?.density ?? window.territoryMapControls?.getTerritoryDensityEnabled?.() ?? false;
  let territoryBlendEnabled = territoryDensityEnabled
    ? false
    : savedSettings?.blend ?? window.territoryMapControls?.getTerritoryBlendEnabled?.() ?? false;
  ...
  const syncTerritoryBrandCirclesToggle = () => {
    territoryBrandCirclesToggle?.setAttribute("aria-checked", String(territoryBrandCirclesEnabled));
  };
  ...
  territoryBrandCirclesToggle?.addEventListener("click", () => {
    territoryBrandCirclesEnabled = !territoryBrandCirclesEnabled;
    syncTerritoryBrandCirclesToggle();
    window.territoryMapControls?.setTerritoryBrandCirclesVisible?.(territoryBrandCirclesEnabled);
    persistTerritorySettings();
  });
  ...
  territoryBlendToggle?.addEventListener("click", () => {
    ...
    if (territoryBlendEnabled && territoryDensityEnabled) {
  ...
  territoryDensityToggle?.addEventListener("click", () => {
    ...
    if (territoryDensityEnabled && territoryBlendEnabled) {
...
function hasNarrowingTerritoryFilters() {
  ...
  return getFilterSelectIncludedValues(locationFilterSelect).length > 0
    || getFilterSelectExcludedValues(locationFilterSelect).length > 0
...
function getAppliedTerritoryFilterCount() {
  ...
  const selectedFilterCount =
    getFilterSelectIncludedValues(locationFilterSelect).length +
    getFilterSelectExcludedValues(locationFilterSelect).length +
...
function applyCrossroadPresetSelections(preset = {}) {
  ...
  setFilterSelectIncludedExcludedValues(
    ...
    getValidSavedSelectValues(locationFilterSelect, preset.locationsExcluded)
  );
  setFilterSelectIncludedExcludedValues(
    categoryFilterSelect,
